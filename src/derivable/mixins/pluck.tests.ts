 ...
 */
export function testPluck(factories: Factories, isSettable: boolean, isAtom: boolean) {
    describe('#pluck', () => {
        it('should pluck using a string or derivable string', () => {
            const obj$ = factories.value({ a: 'valueOfA', b: 'valueOfB' });
            const a$ = obj$.pluck('a');
        ...
        it('should pluck using a Derivable of string or number', () => {
            const arr$ = factories.value(['first', 'second']);
        ...
        it('should pluck immutable values', () => {
            const value$ = factories.value(fromJS({ a: ['value1', 'value2'] }));
        ...
        it('should mirror the type of the input (being SettableDerivable or even DerivableAtom)', () => {
            const value$ = factories.value({ a: 1 });
        ...
        if (isSettable) {
            ...
            beforeEach(() => { value$ = assertSettable(factories.value(new MyClass('value'))); });
            ...
            describe('(lensed with constant key)', () => {
                ...
                it('should produce a lens that can change immutable values', () => {
                    const immValue$ = factories.value(fromJS({ a: ['value1', 'value2'], b: ['value3', 'value4'] })) as Atom<any>;
                ...
                it('should not allow readonly immutable values to be changed', () => {
                    const immValue$ = factories.value(Seq.Indexed.of(1, 2, 3)) as Atom<any>;
